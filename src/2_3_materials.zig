...
pub fn main() !void {
    ...
    const lightingShader = try shader.create(allocator, "shaders/2_3_materials.vert", "shaders/2_3_materials.frag");
    ...
    while (!window.shouldClose()) {
        ...
        var lightColor: zm.Vec = .{ 1.0, 1.0, 1.0, 0.0 };
        lightColor[0] = @sin(currentFrame * 2.0);
        lightColor[1] = @sin(currentFrame * 0.7);
        lightColor[2] = @sin(currentFrame * 1.3);

        const diffuseColor = lightColor * zm.f32x4s(0.5);
        const ambientColor = diffuseColor * zm.f32x4s(0.2);
