...
pub fn main() !void {
    ...
    const ourShader = try shader.create(allocator, "shaders/1_5_shader.vert", "shaders/1_5_shader.frag");
    ...
    while (!window.shouldClose()) {
        ...
        const quat = zm.quatFromAxisAngle(.{ 0.0, 0.0, 1.0, 1.0 }, @floatCast(glfw.getTime()));
        const rot = zm.quatToMat(quat);
        const trans = zm.mul(rot, zm.translation(0.5, -0.5, 0.0));

        const transforLoc = gl.getUniformLocation(ourShader.id, "transform");
        gl.uniformMatrix4fv(transforLoc, 1, gl.FALSE, &zm.matToArr(trans));
