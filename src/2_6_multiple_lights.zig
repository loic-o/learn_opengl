...
pub fn main() !void {
    ...
    const lightingShader = try shader.create(allocator, "shaders/2_6_multiple_lights.vert", "shaders/2_6_multiple_lights.frag");
    ...
    const pointLightPositions = [_][3]f32{
        .{ 0.7, 0.2, 2.0 },
        .{ 2.3, -3.3, -4.0 },
        .{ -4.0, 2.0, -12.0 },
        .{ 0.0, 0.0, -3.0 },
    ...
    while (!window.shouldClose()) {
        ...
        // spotLight
        lightingShader.setVec3("spotLight.position", .{ camera.position[0], camera.position[1], camera.position[2] });
        lightingShader.setVec3("spotLight.direction", .{ camera.front[0], camera.front[1], camera.front[2] });
        ...
        lightingShader.setFloat("spotLight.cutOff", std.math.cos(std.math.degreesToRadians(f32, 12.5)));
        lightingShader.setFloat("spotLight.outerCutOff", std.math.cos(std.math.degreesToRadians(f32, 15.0)));
